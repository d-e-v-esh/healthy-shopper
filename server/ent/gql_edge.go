...
func (pr *Product) ProductItem(ctx context.Context) (*ProductItem, error) {
	...
	if IsNotLoaded(err) {
		result, err = pr.QueryProductItem().Only(ctx)
	}
	return result, MaskNotFound(err)
}

func (pr *Product) Promotion(ctx context.Context) (*Promotion, error) {
...
func (pr *Promotion) Product(ctx context.Context) (result []*Product, err error) {
	if fc := graphql.GetFieldContext(ctx); fc != nil && fc.Field.Alias != "" {
		...
		result, err = pr.Edges.ProductOrErr()
	}
	if IsNotLoaded(err) {
		result, err = pr.QueryProduct().All(ctx)
	}
	return result, err
}
...
func (sm *ShippingMethod) ShopOrder(ctx context.Context) (result []*ShopOrder, err error) {
	if fc := graphql.GetFieldContext(ctx); fc != nil && fc.Field.Alias != "" {
		result, err = sm.NamedShopOrder(graphql.GetFieldContext(ctx).Field.Alias)
	} else {
		result, err = sm.Edges.ShopOrderOrErr()
	}
	if IsNotLoaded(err) {
		result, err = sm.QueryShopOrder().All(ctx)
	}
	return result, err
}
...
func (so *ShopOrder) ShippingMethod(ctx context.Context) (*ShippingMethod, error) {
